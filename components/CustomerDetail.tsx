...
import { Mail, ExternalLink, Calendar, Tag } from 'lucide-react';
import type { SheetRow } from '@/lib/sheets';
import { parseSheetDate, isStandardPlan } from '@/lib/sheets';
import { Dialog, Avatar, DialogStat } from './Dialog';
...
export function CustomerDetail({
  ...
  return (
    <Dialog
      icon={<Avatar name={snap.fullName} size="sm" />}
      title={snap.fullName}
      subtitle={
        <a
          href={`mailto:${snap.email}`}
          className="inline-flex items-center gap-1.5 text-bb-purple hover:underline"
      ...
      stats={
        <>
          <DialogStat label="Plans" value={Array.from(snap.plans).join(', ') || '—'} />
          <DialogStat label="Pricing seen" value={Array.from(snap.pricings).join(', ') || '—'} />
          <DialogStat label="In sheets" value={snap.appearsIn.length.toString()} tone="accent" />
          <DialogStat
            label="Last activity"
            value={
    ...
    >
      <div className="p-6 space-y-6">
        {/* Sheets badges */}
        {snap.appearsIn.length > 0 && (
          <section>
            <div className="text-xs font-semibold text-bb-ink/50 uppercase tracking-wide mb-2">Currently in</div>
            <div className="flex flex-wrap gap-2">
              {snap.appearsIn.map((s) => (
                <span key={s} className="text-xs px-2.5 py-1 rounded-full bg-white border border-bb-ink/10 text-bb-ink">
                  {s}
                </span>
              ))}
            </div>
